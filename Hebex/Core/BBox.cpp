...
namespace Hebex 
{
	...
	void BBox::BoundingSphere(Point3f *c, float *rad) const {
		*c = .5f * pMin + .5f * pMax;
	...
	bool BBox::IntersectP(const Ray &ray, float *hitt0, float *hitt1) const {
		...
		for (int i = 0; i < 3; ++i) {
			...
			float invRayDir = 1.f / ray.mDirection[i];
			float tNear = (pMin[i] - ray.mOrigin[i]) * invRayDir;
			float tFar = (pMax[i] - ray.mOrigin[i]) * invRayDir;
